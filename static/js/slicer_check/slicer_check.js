...
function create_row_html(id,printer_id=1) {
    ...
    $(`#stl_div`).append(
        `<div class="">
            ...
            <div id=stl_row_${id} class="stl_row row border selected_stl shadow-lg p-1 translucent" onclick="select_stl(${id});">
                <!-- 'Select'  --> 
...
function delete_stl(id,printer_id=1) {
    ...
    if ($(`#stl_row_${id}`).hasClass("selected_stl") && Object.keys(stl_list).length > 0) {
...
function select_stl(id,printer_id=1) {
    ...
    if (!row_not_selected_flag) {
        ...
        try {
            ...
            $(`#stl_cont_${previously_selected_row}`).css('display', 'none'); //canvas 

            $(`#stl_row_${previously_selected_row}`).removeClass('shadow-lg selected_stl');
        ...
        $(`#stl_row_${id}`).addClass('selected_stl shadow-lg');
...
function get_available_printer_success_callback(response) {
    ...
    $(`#stl_${id}_length_of_filament`).html(`<small>${stl_list[id]['length_of_filament']}meters</small>`)
    $(`#stl_${id}_time_to_print`).html(`<small>${stl_list[id]['time_to_print'] }seconds</small>`)
